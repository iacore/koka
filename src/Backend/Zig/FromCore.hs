...
--------------------------------------------------------------------------
-- Generate Zig code from System-F core language
--------------------------------------------------------------------------

zigFromCore :: BuildType -> Maybe (Name,Bool) -> [Import] -> Core -> Doc
zigFromCore buildType mbMain imports core
  ...
  where
    ...
    tailCallArg expr
      = let captured = filter (not . isQualified . getName) $ tnamesList $ capturedVar expr
        in if (null captured)
            then return expr
            else -- trace ("Backend.Zig.FromCore.tailCall: capture: " ++ show captured ++ ":\n" ++ show expr) $
...
genMatch result scrutinees branches
  = fmap (debugWrap "genMatch") $ do
    case branches of
        []  -> fail ("Backend.Zig.FromCore.genMatch: no branch in match statement: " ++ show(scrutinees))
...
genExpr expr
  = -- trace ("genExpr: " ++ show expr) $
    case expr of
     ...
     _ -> failure ("Zig.FromCore.genExpr: invalid expression:\n" ++ show expr)
...
genPure expr
  = case expr of
     ...
     _ -> failure ("Zig.FromCore.genPure: invalid expression:\n" ++ show expr)
...
genInline expr
  = case expr of
      ...
      _ -> failure ("Zig.FromCore.genInline: invalid expression:\n" ++ show expr)
...
genExprExternalPrim tname formats argDocs0
  ...
  where
    ...
    ppExternalF name k@('#':'#':xs) args
     = failure ("Backend.Zig.FromCore: type arguments in javascript external in: " ++ show tname)
...
reserved
  = S.fromList $ -- Zig pseudo-keywords
    [ "prototype"
    , "toString"
    , "arguments"
    , "eval"
